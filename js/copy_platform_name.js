...
platformName(window.location.pathname.split('/')[3]).then((result) => {
  ...
  let parent = [...document.querySelectorAll('.ispui-input-group.ispui-input-group_indent_default.ng-star-inserted')]
    .map((el) => el.querySelectorAll('.ispui-label.ispui-label_required')[0])
    .filter((el) => el)
    .filter((el) => el.innerText == 'Платформа')[0].parentNode.parentNode;
  parent.querySelector('.vmdci-select.vmdci-select__wrapper.vmdci-select__value_cursor-pointer.ng-star-inserted').click();

  let imitateInput = () => {
    let inputEl = parent.querySelector('.vmdci-select__input-search');
